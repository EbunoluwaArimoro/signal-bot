...
"""
Rich Genie Signal Bot v2.1 — Bug fixes + Accuracy upgrades
============================================================
FIXES IN THIS VERSION:
  FIX 1 — TP/SL notifications no longer repeat every hour
           Each level (TP1, TP2, TP3, SL) fires ONCE then is marked done
  FIX 2 — Signal message includes ID + timestamp so you always know
           which trade the notification belongs to
  FIX 3 — RSI hard limits: no BUY above RSI 80, no SELL below RSI 20
           Blocks the BNB RSI-95 buy and RSI-12 sell bugs
  FIX 4 — Volume floor: signals blocked if volume < 0.4x average
           Removes the "0.1x volume" phantom signals
  FIX 5 — Daily cap race condition fixed: cap checked AND incremented
           atomically before any signal fires in a batch
  FIX 6 — Signal state machine: tracks tp1_hit, tp2_hit flags
           so the bot knows what stage each trade is at
  FIX 7 — Signals expire cleanly: once TP2 is hit, bot monitors
           for TP3 or SL only — never re-announces TP1/TP2 again
"""
...
CHAT_ID             = os.environ.get("CHAT_ID",         "YOUR_CHAT_ID_HERE")
CAPITAL             = float(os.environ.get("CAPITAL",        "100"))
RISK_PER_TRADE      = float(os.environ.get("RISK_PCT",       "2"))
WEEKLY_TARGET       = float(os.environ.get("WEEKLY_TARGET",  "10"))
SCAN_INTERVAL       = int(os.environ.get("SCAN_INTERVAL",    "3600"))

# Quality gates
MIN_CONDITIONS_MET  = 4      # Min indicators agreeing to fire signal
MIN_RR_RATIO        = 1.8    # Min reward:risk ratio
MAX_SIGNALS_PER_DAY = 3      # Hard daily cap
COOLDOWN_HOURS      = 6      # Hours between signals on same pair
MIN_VOLUME_RATIO    = 0.4    # FIX 4: block signals with < 40% avg volume
RSI_MAX_BUY         = 78     # FIX 3: never buy above this RSI
RSI_MIN_SELL        = 22     # FIX 3: never sell below this RSI
...
state = {
    ...
    "losses_week":      0,
    "last_signal_time": {},
    "open_signals":     [],   # FIX 6: each entry now has tp1_hit, tp2_hit flags
    "week_start":       datetime.now(timezone.utc),
    "day_start":        datetime.now(timezone.utc),
    "capital":          CAPITAL,
    "signal_counter":   0,    # FIX 2: unique ID per signal
}
...
def calc_supertrend(candles, period=10, multiplier=3.0):
    ...
    for i in range(1, len(candles)):
        ...
        atr_vals.append(tr)
    smooth = []
    smooth.append(sum(atr_vals[:period]) / period)
    for v in atr_vals[period:]:
        smooth.append((smooth[-1] * (period-1) + v) / period)
    if len(smooth) < 5:
        return 0, closes[-1]
    hl2        = [(h+l)/2 for h,l in zip(highs[1:], lows[1:])]
    upper_band = [m + multiplier*a for m,a in zip(hl2, smooth)]
    lower_band = [m - multiplier*a for m,a in zip(hl2, smooth)]
    prev_upper = upper_band[-2] if len(upper_band) > 1 else upper_band[-1]
    prev_lower = lower_band[-2] if len(lower_band) > 1 else lower_band[-1]
    prev_close = closes[-2]
    curr_close = closes[-1]
...
def analyse_pair(symbol):
    ...
    # ── FIX 4: Hard volume floor — no signal on dead volume ──────
    if vol_r < MIN_VOLUME_RATIO:
        log.info(f"  {symbol}: volume too low ({vol_r:.2f}x). Skipping.")
        return None

    # ── FIX 3: Hard RSI limits — block dangerous extremes ────────
    # Never buy when RSI is dangerously overbought
    # Never sell when RSI is dangerously oversold (panic dump — not a signal)
    buy_rsi_blocked  = rsi_1h > RSI_MAX_BUY
    sell_rsi_blocked = rsi_1h < RSI_MIN_SELL
    ...
    if price <= bb_lo * 1.005:
        buy_conditions.append("Price at BB lower (value zone)")
    elif price < bb_mid:
        buy_conditions.append("Price below BB midline")
    if vol_r >= 1.3:
        buy_conditions.append(f"Volume spike ({vol_r:.1f}x avg)")
    if ema9 > ema21:
        buy_conditions.append("EMA bullish alignment")
    ...
    if vol_r >= 1.3:
        sell_conditions.append(f"Volume spike ({vol_r:.1f}x avg)")
    if ema9 < ema21:
        sell_conditions.append("EMA bearish alignment")
    ...
    # ── APPLY RSI HARD BLOCKS ────────────────────────────────────
    if buy_rsi_blocked:
        buy_conditions = []
        log.info(f"  {symbol}: BUY blocked — RSI {rsi_1h:.0f} too overbought (>{RSI_MAX_BUY})")
    if sell_rsi_blocked:
        sell_conditions = []
        log.info(f"  {symbol}: SELL blocked — RSI {rsi_1h:.0f} too oversold (<{RSI_MIN_SELL})")

    # ── DETERMINE DIRECTION ──────────────────────────────────────
    buy_c  = len(buy_conditions)
    sell_c = len(sell_conditions)
    ...
    return {
        ...
        "atr":         round(atr, 6),
        "fired_at":    datetime.now(timezone.utc),
        # FIX 1 & 6 — state flags so each notification fires exactly once
        "tp1_hit":     False,
        "tp2_hit":     False,
        "tp3_hit":     False,
        "sl_hit":      False,
...
# ─── OUTCOME TRACKING (FIX 1 + FIX 6) ──────────────────────────

def check_open_signals():
    """
    FIX 1: Each TP/SL level fires its notification EXACTLY ONCE.
    Uses state flags (tp1_hit, tp2_hit, etc.) on each signal dict.
    Signal is removed from open_signals only when fully closed.
    """
    to_remove = []
    ...
    for sig in state["open_signals"]:
        ...
        try:
            ...
            # Check each level — only fire if not already notified
            tp3_reached = (d == "BUY"  and price >= sig["tp3"]) or \
                          (d == "SELL" and price <= sig["tp3"])
            tp2_reached = (d == "BUY"  and price >= sig["tp2"]) or \
                          (d == "SELL" and price <= sig["tp2"])
            tp1_reached = (d == "BUY"  and price >= sig["tp1"]) or \
                          (d == "SELL" and price <= sig["tp1"])
            sl_reached  = (d == "BUY"  and price <= sig["sl"])  or \
                          (d == "SELL" and price >= sig["sl"])

            # TP3 — final target
            if tp3_reached and not sig["tp3_hit"]:
                sig["tp3_hit"] = True
                sig["tp2_hit"] = True  # mark lower levels done too
                sig["tp1_hit"] = True
                gain_pct  = abs(sig["tp3"] - sig["entry"]) / sig["entry"] * 100
                gain_usd  = round(sig["pos_value"] * gain_pct / 100, 2)
                send_telegram(
                    f"🏆 <b>TP3 HIT {sid} — {sig['display']}</b>\n"
                    f"━━━━━━━━━━━━━━━━━━━━━━━\n"
                    f"Entry:  {fmt(sig['entry'])}\n"
                    f"TP3:    {fmt(sig['tp3'])}  (R:R 1:{sig['rr3']})\n"
                    f"Est. profit: +${gain_usd}\n"
                    f"━━ Close full position. Outstanding trade. ✅"
                )
                sig["closed"] = True
                state["wins_week"] += 1
                to_remove.append(sig)

            # TP2 — fire once, then monitor for TP3
            elif tp2_reached and not sig["tp2_hit"]:
                sig["tp2_hit"] = True
                sig["tp1_hit"] = True
                send_telegram(
                    f"🎯 <b>TP2 HIT {sid} — {sig['display']}</b>\n"
                    f"━━━━━━━━━━━━━━━━━━━━━━━\n"
                    f"Entry:  {fmt(sig['entry'])}\n"
                    f"TP2:    {fmt(sig['tp2'])}  (R:R 1:{sig['rr2']})\n"
                    f"Action: Close 40% now. Move SL to entry (risk-free).\n"
                    f"Watching for TP3 at {fmt(sig['tp3'])} 👀"
                )

            # TP1 — fire once, then monitor for TP2
            elif tp1_reached and not sig["tp1_hit"]:
                sig["tp1_hit"] = True
                send_telegram(
                    f"✅ <b>TP1 HIT {sid} — {sig['display']}</b>\n"
                    f"━━━━━━━━━━━━━━━━━━━━━━━\n"
                    f"Entry:  {fmt(sig['entry'])}\n"
                    f"TP1:    {fmt(sig['tp1'])}  (R:R 1:{sig['rr1']})\n"
                    f"Action: Close 40% now. Move SL to entry (break-even).\n"
                    f"Watching for TP2 at {fmt(sig['tp2'])} 👀"
                )

            # SL — only fires once, then signal is closed
            elif sl_reached and not sig["sl_hit"]:
                sig["sl_hit"] = True
                send_telegram(
                    f"🛑 <b>STOP LOSS HIT {sid} — {sig['display']}</b>\n"
                    f"━━━━━━━━━━━━━━━━━━━━━━━\n"
                    f"Entry:  {fmt(sig['entry'])}\n"
                    f"SL:     {fmt(sig['sl'])}\n"
                    f"Loss:   -${sig['risk_usd']} ({RISK_PER_TRADE}% of capital)\n"
                    f"Capital protected. Next setup incoming. 📊"
                )
                sig["closed"] = True
                state["losses_week"] += 1
                to_remove.append(sig)

            # Expire after 48 hours without resolution
            age_h = (datetime.now(timezone.utc) - sig["fired_at"]).total_seconds() / 3600
            if age_h > 48 and not sig["closed"]:
                tp_label = "TP1" if sig["tp1_hit"] else "none"
                send_telegram(
                    f"⏰ <b>SIGNAL EXPIRED {sid} — {sig['display']}</b>\n"
                    f"Signal open 48h without full resolution.\n"
                    f"Best level hit: {tp_label}. Closing tracking."
                )
                sig["closed"] = True
                to_remove.append(sig)

        except Exception as e:
            log.warning(f"Outcome check error {sig.get('id','?')}: {e}")
...
def can_send_signal(symbol):
    # FIX 5: cap checked before any signal fires — prevents batch race condition
    if state["signals_today"] >= MAX_SIGNALS_PER_DAY:
        return False
    last = state["last_signal_time"].get(symbol)
    if last:
...
def build_signal_message(sig):
    ...
    return (
        f"{arrow} <b>SIGNAL {sig['id']} — {sig['display']}</b>\n"
        f"━━━━━━━━━━━━━━━━━━━━━━━\n"
        f"📅 {now}\n"
        f"⚡ Direction: <b>{sig['direction']}</b>\n"
        f"🌍 Market: {regime_label(sig['regime'])}\n"
        f"💪 Confidence: {sig['confidence']}% [{bar}]\n"
        f"📊 Conditions: {sig['score']}/7\n\n"
        f"<b>📍 ENTRY</b>\n"
        f"{fmt(sig['entry'])}\n\n"
        f"<b>🛑 STOP LOSS</b>\n"
        f"{fmt(sig['sl'])}  (max loss ${sig['risk_usd']})\n\n"
        f"<b>🎯 TAKE PROFITS</b>\n"
        f"TP1 → {fmt(sig['tp1'])}  R:R 1:{sig['rr1']}  (close 40%)\n"
        ...
        f"Spend: ${sig['pos_value']} | Qty: {sig['qty']} {sig['symbol'].replace('USDT','')}\n"
        f"Max risk: ${sig['risk_usd']}\n\n"
        f"<b>📋 WHY THIS SIGNAL</b>\n"
        f"{conds}\n\n"
        f"RSI 1H: {sig['rsi_1h']} | RSI 4H: {sig['rsi_4h']}\n"
        f"ST 1H: {st_bar(sig['st_dir_1h'])} | ST 4H: {st_bar(sig['st_dir_4h'])}\n"
        f"Vol: {sig['vol_ratio']}x avg | BB: {sig['bb_bw']}% | 24h: {sig['change_24h']:+.1f}%\n"
        f"━━━━━━━━━━━━━━━━━━━━━━━\n"
        f"⚠️ Analysis only. Always use your SL."
    )
...
def main():
    log.info("Rich Genie Bot v2.1 starting...")
    send_telegram(
        f"🤖 <b>Rich Genie Bot v2.1 LIVE</b>\n"
        f"━━━━━━━━━━━━━━━━━━━━━━━\n"
        f"🔬 Accuracy Engine (all 6 bugs fixed)\n"
        f"  ✓ Supertrend 1H + 4H\n"
        f"  ✓ MACD + RSI + Bollinger Bands\n"
        f"  ✓ Volume floor: min {MIN_VOLUME_RATIO}x avg\n"
        f"  ✓ RSI hard limits: buy <{RSI_MAX_BUY}, sell >{RSI_MIN_SELL}\n"
        f"  ✓ Market regime filter\n"
        f"  ✓ 6h cooldown per pair\n"
        f"  ✓ Max {MAX_SIGNALS_PER_DAY} signals/day\n"
        f"  ✓ TP/SL notifications fire ONCE each (fixed)\n"
        f"  ✓ Signal IDs so you always know which trade\n\n"
        f"💰 ${CAPITAL} capital | {RISK_PER_TRADE}% risk | {WEEKLY_TARGET}% weekly target\n"
        f"Scanning {len(PAIRS)} pairs every {SCAN_INTERVAL//60} min\n"
        f"━━━━━━━━━━━━━━━━━━━━━━━\n"
        f"Standing by..."
    )
    ...
    while True:
        ...
        # FIX 5: Check cap once before the loop, not inside it
        if state["signals_today"] >= MAX_SIGNALS_PER_DAY:
            log.info(f"Daily cap ({MAX_SIGNALS_PER_DAY}) already reached. Skipping scan.")
        else:
            for symbol in PAIRS:
                # Re-check cap inside loop too (in case it fills mid-loop)
                if state["signals_today"] >= MAX_SIGNALS_PER_DAY:
                    log.info("Daily cap reached mid-scan. Stopping.")
                    break
                ...
                if sig:
                    # FIX 5: increment BEFORE sending so cap is respected immediately
                    state["signals_today"]            += 1
                    state["signals_week"]             += 1
                    ...
                    msg = build_signal_message(sig)
                    send_telegram(msg)
                    log.info(f"  ✓ Signal {sig['id']}: {symbol} {sig['direction']} | {sig['score']}/7 | conf {sig['confidence']}%")
                    time.sleep(2)
                else:
                    log.info(f"  ✗ {symbol}: no quality setup")
